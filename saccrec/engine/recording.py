...
puerto = '/dev/ttyUSB0'
port = serial.Serial(port=puerto,baudrate=115200,timeout=2)
...
def _run(board):
    cycle = 0.85 * board.cycle #board.cycle: tiempo secs en tomar una muestra
    unit_wait = cycle / 10.0
    last_acquired = time.monotonic()
    while True:
        now = time.monotonic() #monotonic(): devuelve el tiempo secs.mili del sistema
        if now - last_acquired < cycle:
            time.sleep(unit_wait) # delay de espera para la proxima muestra
            continue
        sample = board.read_sample()
        last_acquired = now
