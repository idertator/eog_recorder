...
from saccrec.core.enums import Gain, Language
...
class _GUISettingsPage(QtWidgets.QWidget):
    ...
    def save(self):
        ...
        if lang != self._initial_lang:
            ...
            answer.setIcon(QtWidgets.QMessageBox.Question)
            answer.setWindowTitle(_("Alert"))
            answer.setText(
                _(
                    "The app must be restarted to apply the changes. Do you want to close the app?"
                )
            )
...
class _OpenBCIChannelWidget(QtWidgets.QFrame):
    ...
    def _on_activated_changed(self):
        self._gain_combo.setVisible(self._activated_check.isChecked())
        self._gain_label.setVisible(self._activated_check.isChecked())
        self._srb1_check.setVisible(self._activated_check.isChecked())
        self._srb2_check.setVisible(self._activated_check.isChecked())
        self.activationChanged.emit()

    @property
    def active(self) -> bool:
        return self._activated_check.isChecked()
